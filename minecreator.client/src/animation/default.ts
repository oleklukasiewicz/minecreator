...
import { AnimationPropertyStep, AnimationStep } from "$src/helpers/render/animationHelper";
import { CreateModelAnimationData } from "$src/helpers/render/modelAnimationData";
const DefaultAnimation = new RenderAnimation(
  async function (scene, keepData = false, modelName) {
    let data: any = await CreateModelAnimationData(scene, modelName, false);
    // Keep pivots attached to body to preserve original placement
    if (data.leftLegPivot) data.leftLegPivot.rotation.x = -0.1;
  ...
  function (data, scene, clock, modelName, elapsedRenderTime) {
    ...
    const ert = typeof elapsedRenderTime === "number" ? elapsedRenderTime : 0;
    const cSin = Math.sin(ert);
    AnimationStep(
      data,
      [
    ...
    if (data.head) {
      if (ert > data.nextRotationTime) {
        // Time for a new rotation
        data.headRotation = ((Math.random() * 120 - 60) * Math.PI) / 180; // New random rotation
        data.nextRotationTime = ert + Math.random() * 5; // New random time for the next rotation
