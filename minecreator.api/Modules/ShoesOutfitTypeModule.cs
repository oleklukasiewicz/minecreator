...
namespace minecreator.api.Modules
{
    public class ShoesOutfitTypeModule : OutfitModule
    {
        ...
        public override TextureMap GenerateBaseTexture()
        {
            ...
            var materialCharacteristic = Workspace.Characteristics.Material % 2;
        ...
        private TextureMapFullPart ProcessLegPart(OutfityTypeCharacteristics characteristics, Image<Rgba32> innerpart, Image<Rgba32> outerpart, TexturePattern pattern)
        {
            ...
            if (Configuration.Style == OutfitStyle.SUMMER)
                lengthCharacteristic += 2;

            var shoeBaseY = 13;
            var shoesStartY = 4;

            var movableAreaHeight = shoeBaseY - shoesStartY;


            var movingPartHeight = movableAreaHeight - lengthCharacteristic;

            //if length==4 - cut the area from texture
            if (lengthCharacteristic > 4)
            {
                innerpart = TextureManupulationHelper.CutRectangle(innerpart,new Rectangle(0, shoesStartY, innerpart.Width, shoeBaseY- shoesStartY));
            }
            else
            {
                TextureManupulationHelper.MoveByVector(
                    innerpart,
                    new Rectangle(0, shoesStartY, innerpart.Width, movingPartHeight),
                    new Point(0, lengthCharacteristic)
                );
            }
        ...
        public override TextureMap GenerateDetailsTexture()
        {
            ...
            if (detailsCharacterists == 1)
            {
                ...
                var leftlegBorderRectangle = new Rectangle(0, 15, leftlegOuterpart.Width, 1);
                ...
                var rightlegBorderRectangle = new Rectangle(0, 15, rightlegOuterpart.Width, 1);
